...
import { trackCommands } from "./commandsTrackChanges";
...
export const createYjsStore = (ctx: EditorProviders, opts: YjsOptions) => {
  ...
  const provider =
    initial?.provider || new WebsocketProvider(ws_url, document.id, ydoc);
  ...
  return {
    ...
    getState(): YjsExtensionState {
      ...
      const changed = Object.keys(newState).some(
        (key: string) =>
          prevState[key as keyof YjsExtensionState] !==
          newState[key as keyof YjsExtensionState]
      );
      if (changed) {
        prevState = newState;
        return newState;
